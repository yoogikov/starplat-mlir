...
template <typename... OpTy> static bool matchSimpleReduction(Block& block) {
    if (block.empty() || llvm::hasSingleElement(block) || std::next(block.begin(), 2) != block.end())
        return false;
    ...
    return isa<OpTy...>(combinerOps[0]) && isa<scf::ReduceReturnOp>(block.back()) && block.front().getOperands() == block.getArguments();
}
...
static bool matchSelectReduction(Block& block, ArrayRef<Predicate> lessThanPredicates, ArrayRef<Predicate> greaterThanPredicates, bool& isMin) {
    static_assert(llvm::is_one_of<SelectOpTy, arith::SelectOp, LLVM::SelectOp>::value, "only arithmetic and llvm select ops are supported");

    // Expect exactly three operations in the block.
    if (block.empty() || llvm::hasSingleElement(block) || std::next(block.begin(), 2) == block.end() || std::next(block.begin(), 3) != block.end())
        return false;
    ...
    else if (llvm::is_contained(greaterThanPredicates, compare.getPredicate())) {
        isLess = false;
    }
    else {
        return false;
    ...
    bool sameOperands              = select.getOperand(kTrueValue) == compare.getLhs() && select.getOperand(kFalseValue) == compare.getRhs();
    bool swappedOperands           = select.getOperand(kTrueValue) == compare.getRhs() && select.getOperand(kFalseValue) == compare.getLhs();
...
static Attribute minMaxValueForFloat(Type type, bool min) {
    ...
    return FloatAttr::get(type, llvm::APFloat::getLargest(fltSemanticsForType(fltType), min));
}
...
static Attribute minMaxValueForSignedInt(Type type, bool min) {
    ...
    return IntegerAttr::get(type, min ? llvm::APInt::getSignedMinValue(bitwidth) : llvm::APInt::getSignedMaxValue(bitwidth));
}
...
static Attribute minMaxValueForUnsignedInt(Type type, bool min) {
    auto intType      = cast<IntegerType>(type);
    unsigned bitwidth = intType.getWidth();
    return IntegerAttr::get(type, min ? llvm::APInt::getZero(bitwidth) : llvm::APInt::getAllOnes(bitwidth));
}
...
/// over from `reduce`.
static omp::DeclareReductionOp createDecl(PatternRewriter& builder, SymbolTable& symbolTable, scf::ReduceOp reduce, int64_t reductionIndex,
                                          Attribute initValue) {
    OpBuilder::InsertionGuard guard(builder);
    Type type = reduce.getOperands()[reductionIndex].getType();
    auto decl = omp::DeclareReductionOp::create(builder, reduce.getLoc(), "__scf_reduction", type,
                                                /*byref_element_type=*/{});
    symbolTable.insert(decl);

    builder.createBlock(&decl.getInitializerRegion(), decl.getInitializerRegion().end(), {type}, {reduce.getOperands()[reductionIndex].getLoc()});
    builder.setInsertionPointToEnd(&decl.getInitializerRegion().back());
    Value init = LLVM::ConstantOp::create(builder, reduce.getLoc(), type, initValue);
    omp::YieldOp::create(builder, reduce.getLoc(), init);

    Operation* terminator = &reduce.getReductions()[reductionIndex].front().back();
    assert(isa<scf::ReduceReturnOp>(terminator) && "expected reduce op to be terminated by redure return");
    builder.setInsertionPoint(terminator);
    builder.replaceOpWithNewOp<omp::YieldOp>(terminator, terminator->getOperands());
    builder.inlineRegionBefore(reduce.getReductions()[reductionIndex], decl.getReductionRegion(), decl.getReductionRegion().end());
    return decl;
}

/// Adds an atomic reduction combiner to the given OpenMP reduction declaration
/// using llvm.atomicrmw of the given kind.
static omp::DeclareReductionOp addAtomicRMW(OpBuilder& builder, LLVM::AtomicBinOp atomicKind, omp::DeclareReductionOp decl, scf::ReduceOp reduce,
                                            int64_t reductionIndex) {
    OpBuilder::InsertionGuard guard(builder);
    auto ptrType              = LLVM::LLVMPointerType::get(builder.getContext());
    Location reduceOperandLoc = reduce.getOperands()[reductionIndex].getLoc();
    builder.createBlock(&decl.getAtomicReductionRegion(), decl.getAtomicReductionRegion().end(), {ptrType, ptrType},
    ...
    Value loaded = LLVM::LoadOp::create(builder, reduce.getLoc(), decl.getType(), atomicBlock->getArgument(1));
    LLVM::AtomicRMWOp::create(builder, reduce.getLoc(), atomicKind, atomicBlock->getArgument(0), loaded, LLVM::AtomicOrdering::monotonic);
...
static omp::DeclareReductionOp declareReduction(PatternRewriter& builder, scf::ReduceOp reduce, int64_t reductionIndex) {
    ...
    assert(llvm::hasSingleElement(reduce.getReductions()[reductionIndex]) && "expected reduction region to have a single element");
    ...
    if (matchSimpleReduction<arith::AddFOp, LLVM::FAddOp>(reduction)) {
        omp::DeclareReductionOp decl = createDecl(builder, symbolTable, reduce, reductionIndex, builder.getFloatAttr(type, 0.0));
        return addAtomicRMW(builder, LLVM::AtomicBinOp::fadd, decl, reduce, reductionIndex);
    }
    if (matchSimpleReduction<arith::AddIOp, LLVM::AddOp>(reduction)) {
        omp::DeclareReductionOp decl = createDecl(builder, symbolTable, reduce, reductionIndex, builder.getIntegerAttr(type, 0));
        return addAtomicRMW(builder, LLVM::AtomicBinOp::add, decl, reduce, reductionIndex);
    }
    if (matchSimpleReduction<arith::OrIOp, LLVM::OrOp>(reduction)) {
        omp::DeclareReductionOp decl = createDecl(builder, symbolTable, reduce, reductionIndex, builder.getIntegerAttr(type, 0));
        return addAtomicRMW(builder, LLVM::AtomicBinOp::_or, decl, reduce, reductionIndex);
    }
    if (matchSimpleReduction<arith::XOrIOp, LLVM::XOrOp>(reduction)) {
        omp::DeclareReductionOp decl = createDecl(builder, symbolTable, reduce, reductionIndex, builder.getIntegerAttr(type, 0));
        return addAtomicRMW(builder, LLVM::AtomicBinOp::_xor, decl, reduce, reductionIndex);
    }
    if (matchSimpleReduction<arith::AndIOp, LLVM::AndOp>(reduction)) {
        omp::DeclareReductionOp decl = createDecl(builder, symbolTable, reduce, reductionIndex,
                                                  builder.getIntegerAttr(type, llvm::APInt::getAllOnes(type.getIntOrFloatBitWidth())));
        return addAtomicRMW(builder, LLVM::AtomicBinOp::_and, decl, reduce, reductionIndex);
    }
    ...
    if (matchSimpleReduction<arith::MulFOp, LLVM::FMulOp>(reduction)) {
        return createDecl(builder, symbolTable, reduce, reductionIndex, builder.getFloatAttr(type, 1.0));
    }
    if (matchSimpleReduction<arith::MulIOp, LLVM::MulOp>(reduction)) {
        return createDecl(builder, symbolTable, reduce, reductionIndex, builder.getIntegerAttr(type, 1));
    }

    // Match select-based min/max reductions.
    bool isMin;
    if (matchSelectReduction<arith::CmpFOp, arith::SelectOp>(reduction, {arith::CmpFPredicate::OLT, arith::CmpFPredicate::OLE},
                                                             {arith::CmpFPredicate::OGT, arith::CmpFPredicate::OGE}, isMin) ||
        matchSelectReduction<LLVM::FCmpOp, LLVM::SelectOp>(reduction, {LLVM::FCmpPredicate::olt, LLVM::FCmpPredicate::ole},
                                                           {LLVM::FCmpPredicate::ogt, LLVM::FCmpPredicate::oge}, isMin)) {
        return createDecl(builder, symbolTable, reduce, reductionIndex, minMaxValueForFloat(type, !isMin));
    }
    if (matchSelectReduction<arith::CmpIOp, arith::SelectOp>(reduction, {arith::CmpIPredicate::slt, arith::CmpIPredicate::sle},
                                                             {arith::CmpIPredicate::sgt, arith::CmpIPredicate::sge}, isMin) ||
        matchSelectReduction<LLVM::ICmpOp, LLVM::SelectOp>(reduction, {LLVM::ICmpPredicate::slt, LLVM::ICmpPredicate::sle},
                                                           {LLVM::ICmpPredicate::sgt, LLVM::ICmpPredicate::sge}, isMin)) {
        omp::DeclareReductionOp decl = createDecl(builder, symbolTable, reduce, reductionIndex, minMaxValueForSignedInt(type, !isMin));
        return addAtomicRMW(builder, isMin ? LLVM::AtomicBinOp::min : LLVM::AtomicBinOp::max, decl, reduce, reductionIndex);
    }
    if (matchSelectReduction<arith::CmpIOp, arith::SelectOp>(reduction, {arith::CmpIPredicate::ult, arith::CmpIPredicate::ule},
                                                             {arith::CmpIPredicate::ugt, arith::CmpIPredicate::uge}, isMin) ||
        matchSelectReduction<LLVM::ICmpOp, LLVM::SelectOp>(reduction, {LLVM::ICmpPredicate::ugt, LLVM::ICmpPredicate::ule},
                                                           {LLVM::ICmpPredicate::ugt, LLVM::ICmpPredicate::uge}, isMin)) {
        omp::DeclareReductionOp decl = createDecl(builder, symbolTable, reduce, reductionIndex, minMaxValueForUnsignedInt(type, !isMin));
        return addAtomicRMW(builder, isMin ? LLVM::AtomicBinOp::umin : LLVM::AtomicBinOp::umax, decl, reduce, reductionIndex);
    }
...
struct ParallelOpLowering : public OpRewritePattern<scf::ParallelOp>
{
    ...
    ParallelOpLowering(MLIRContext* context, unsigned numThreads = kUseOpenMPDefaultNumThreads)
    ...
    LogicalResult matchAndRewrite(scf::ParallelOp parallelOp, PatternRewriter& rewriter) const override {
        ...
        for (int64_t i = 0, e = parallelOp.getNumReductions(); i < e; ++i) {
            ...
            if (!decl)
                return failure();
            reductionSyms.push_back(SymbolRefAttr::get(rewriter.getContext(), decl.getSymName()));
        }
        ...
        Value one    = LLVM::ConstantOp::create(rewriter, loc, rewriter.getIntegerType(64), rewriter.getI64IntegerAttr(1));
        ...
        for (Value init : parallelOp.getInitVals()) {
            assert((LLVM::isCompatibleType(init.getType()) || isa<LLVM::PointerElementTypeInterface>(init.getType())) &&
                   "cannot create a reduction variable if the type is not an LLVM "
                   "pointer element");
            Value storage = LLVM::AllocaOp::create(rewriter, loc, ptrType, init.getType(), one, 0);
        ...
        // reduction variables.
        for (auto [x, y, rD] : llvm::zip_equal(reductionVariables, reduce.getOperands(), ompReductionDecls)) {
            ...
            assert(redRegion.hasOneBlock() && "expect reduction region to have one block");
            Value pvtRedVar = parallelOp.getRegion().addArgument(x.getType(), loc);
            Value pvtRedVal = LLVM::LoadOp::create(rewriter, reduce.getLoc(), rD.getType(), pvtRedVar);
            ...
            assert(redRegion.getNumArguments() == 2 && "expect reduction region to have two arguments");
            ...
            for (auto& op : redRegion.getOps()) {
                ...
                if (auto yieldOp = dyn_cast<omp::YieldOp>(*cloneOp)) {
                    assert(yieldOp && yieldOp.getResults().size() == 1 && "expect YieldOp in reduction region to return one result");
        ...
        Value numThreadsVar;
        if (numThreads > 0) {
            numThreadsVar = LLVM::ConstantOp::create(rewriter, loc, rewriter.getI32IntegerAttr(numThreads));
        }
        // Create the parallel wrapper.
        auto ompParallel = omp::ParallelOp::create(rewriter, loc,
                                                   ...
                                                   /* reduction_syms = */ ArrayAttr{});
        {
            ...
            // Replace the loop.
            {
                ...
                if (!reductionVariables.empty()) {
                    wsloopOp.setReductionSymsAttr(ArrayAttr::get(rewriter.getContext(), reductionSyms));
                    ...
                    wsloopOp.setReductionByref(DenseBoolArrayAttr::get(rewriter.getContext(), reductionByRef));
                }
                ...
                llvm::transform(reductionVariables, std::back_inserter(reductionTypes), [](mlir::Value v) { return v.getType(); });
                rewriter.createBlock(&wsloopOp.getRegion(), {}, reductionTypes,
                                     llvm::SmallVector<mlir::Location>(reductionVariables.size(), parallelOp.getLoc()));

                // Create loop nest and populate region with contents of scf.parallel.
                auto loopOp = omp::LoopNestOp::create(rewriter, parallelOp.getLoc(), parallelOp.getLowerBound().size(), parallelOp.getLowerBound(),
                                                      parallelOp.getUpperBound(), parallelOp.getStep(), /*loop_inclusive=*/false,
                                                      /*tile_sizes=*/nullptr);

                rewriter.inlineRegionBefore(parallelOp.getRegion(), loopOp.getRegion(), loopOp.getRegion().begin());
                ...
                rewriter.replaceAllUsesWith(loopOpEntryBlock.getArguments().drop_front(numLoops), wsloopOp.getRegion().getArguments());
                loopOpEntryBlock.eraseArguments(numLoops, loopOpEntryBlock.getNumArguments() - numLoops);

                Block* ops = rewriter.splitBlock(&loopOpEntryBlock, loopOpEntryBlock.begin());

                // Merge ops back into the entry block FIRST (no yield yet)
                rewriter.mergeBlocks(ops, &loopOpEntryBlock, /*argValues=*/ValueRange());

                // NOW add omp.yield at the end, after all the ops
                rewriter.setInsertionPointToEnd(&loopOpEntryBlock);
        ...
        for (auto [variable, type] : llvm::zip(reductionVariables, parallelOp.getResultTypes())) {
...
struct SCFToOpenMPPass : public impl::ConvertSCFToOpenMPPassBase<SCFToOpenMPPass>
{
