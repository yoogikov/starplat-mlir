...
void StarPlatCodeGen::visitDeclarationStmt(const DeclarationStatement *dclstmt,
  ...
  auto declareOp = mlir::starplat::DeclareOp2::create(
      builder, builder.getUnknownLoc(), type,
      builder.getStringAttr(Id->getname()), builder.getStringAttr("public"));
  ...
  if (Num) {
    ...
    if (Num->isFloat()) {
      rhs = mlir::starplat::ConstOp::create(
          builder, builder.getUnknownLoc(), builder.getF64Type(),
          builder.getF64FloatAttr(Num->getnumberfloat()),
          "const_" + to_string(get_const_count()),
      ...
      rhs = mlir::starplat::ConstOp::create(
          ...
          builder.getI64IntegerAttr(Num->getnumber()),
          "const_" + to_string(get_const_count()),
          builder.getStringAttr("private"));
    }

    mlir::starplat::AssignmentOp::create(builder, builder.getUnknownLoc(), lhs,
                                         rhs);
  }
  ...
  // if(std::string(Type->getType())=="int")
  // llvm::errs() <<
  // std::string(static_cast<TemplateType*>(dclstmt->gettype())->getType());
  // auto declareOp = mlir::starplat::DeclareOp::create(builder,
  // builder.getUnknownLoc(), type,
  // builder.getStringAttr(identifier->getname()), visibility, graph);
  // llvm::errs() << "decl happening\n"; mlir::starplat
}

void StarPlatCodeGen::visitTemplateDeclarationStmt(
    const TemplateDeclarationStatement *templateDeclStmt,
    mlir::SymbolTable *symbolTable) {

  TemplateType *Type = static_cast<TemplateType *>(templateDeclStmt->gettype());
  Identifier *identifier =
      static_cast<Identifier *>(templateDeclStmt->getvarname());
  ...
  if (std::string(Type->getGraphPropNode()->getPropertyType()) == "propNode") {
    ...
    if (std::string(Type->getType()->getType()) == "int")
      type = builder.getType<mlir::starplat::PropNodeType>(builder.getI64Type(),
                                                           graphId);
    else if (std::string(Type->getType()->getType()) == "bool")
      type = builder.getType<mlir::starplat::PropNodeType>(builder.getI1Type(),
                                                           graphId);
    else if (std::string(Type->getType()->getType()) == "float")
      type = builder.getType<mlir::starplat::PropNodeType>(builder.getF64Type(),
                                                           graphId);
    else {
    ...
    if (graph)
      declareOp = mlir::starplat::DeclareOp::create(
          builder, builder.getUnknownLoc(), type,
          builder.getStringAttr(identifier->getname()), visibility, graph);
    else
  ...
  else if (std::string(Type->getGraphPropNode()->getPropertyType()) ==
           "propEdge") {

    auto type = builder.getType<mlir::starplat::PropEdgeType>(
        builder.getI64Type(), graphId);
    mlir::Value graph = NULL;
    auto declare = mlir::starplat::DeclareOp::create(
        builder, builder.getUnknownLoc(), type,
...
void StarPlatCodeGen::visitTemplateType(const TemplateType *templateType,
                                        mlir::SymbolTable *symbolTable) {}

void StarPlatCodeGen::visitForallStmt(const ForallStatement *forAllStmt,
                                      mlir::SymbolTable *symbolTable) {

  Identifier *loopVar = static_cast<Identifier *>(forAllStmt->getLoopVar());
  const Expression *expr =
      static_cast<const Expression *>(forAllStmt->getexpr());
  const Statementlist *stmtlist =
      static_cast<const Statementlist *>(forAllStmt->getstmtlist());

  if (globalLookupOp(loopVar->getname())) {
    llvm::outs() << "Error: Identifier '" << loopVar->getname()
                 << "' already declared.\n";
    exit(0);
  }
  ...
  const Memberaccess *memberaccess =
      static_cast<const Memberaccess *>(expr->getExpression());
  const Methodcall *outermethodcall =
      static_cast<const Methodcall *>(memberaccess->getMethodCall());
  ...
  if (memberaccess->getMemberAccessNode()) {
    const Memberaccess *inner =
        static_cast<const Memberaccess *>(memberaccess->getMemberAccessNode());
    const Methodcall *innerMethod =
        static_cast<const Methodcall *>(inner->getMethodCall());
    ...
    if (strcmp(innerMethod->getIdentifier()->getname(), "neighbors") == 0) {
      isNeighbours = true;
      const Expression *paramExpr =
          static_cast<const Expression *>(innerMethod->getParamLists());
      const Identifier *paramId =
          static_cast<const Identifier *>(paramExpr->getExpression());
      nodeSymbol = globalLookupOp(paramId->getname());
      if (!nodeSymbol) {
        llvm::outs() << "Error: Node '" << paramId->getname()
                     << "' not found.\n";
        return;
      }
      ...
      llvm::outs() << "Error: Unknown method '"
                   << innerMethod->getIdentifier()->getname() << "'.\n";
      return;
    }
    ...
    if (outermethodcall && outermethodcall->getIsBuiltin() &&
      ...
      const Expression *filterExpr =
          static_cast<const Expression *>(outermethodcall->getParamLists());
      const BoolExpr *boolExpr =
          static_cast<const BoolExpr *>(filterExpr->getExpression());
      const Expression *lhsExpr =
          static_cast<const Expression *>(boolExpr->getExpr1());
      const Expression *rhsExpr =
          static_cast<const Expression *>(boolExpr->getExpr2());
      filterOp = std::string(boolExpr->getop());
      lhsExprKind = lhsExpr->getKind();

      // LHS
      if (lhsExpr->getKind() == ExpressionKind::KIND_MEMBERACCESS) {
        const Memberaccess *lhsMem =
            static_cast<const Memberaccess *>(lhsExpr->getExpression());
        const Identifier *nodeId =
            static_cast<const Identifier *>(lhsMem->getIdentifier());
        const Identifier *propId =
            static_cast<const Identifier *>(lhsMem->getIdentifier2());
        if (!nodeId || !propId) {
        ...
        if (!lhsPropVal) {
          llvm::outs() << "Error: Undefined property '" << propId->getname()
                       << "' in filter.\n";
          return;
        }
        ...
        const Identifier *lhsId =
            static_cast<const Identifier *>(lhsExpr->getExpression());
      ...
      if (rhsExpr->getKind() == ExpressionKind::KIND_IDENTIFIER) {
        const Identifier *rhsId =
            static_cast<const Identifier *>(rhsExpr->getExpression());
        ...
        const Keyword *rhsKw =
            static_cast<const Keyword *>(rhsExpr->getExpression());
  ...
  // ---- SIMPLE: g.nodes() or g.neighbors(v) ----
    ...
    if (strcmp(method->getIdentifier()->getname(), "nodes") == 0) {
      ...
      isNeighbours = true;
      const Expression *paramExpr =
          static_cast<const Expression *>(method->getParamLists());
      const Identifier *paramId =
          static_cast<const Identifier *>(paramExpr->getExpression());
      nodeSymbol = globalLookupOp(paramId->getname());
      if (!nodeSymbol) {
  ...
  if (!isNeighbours) {
    loopOp = mlir::starplat::ForAllNodesOp::create(
        builder, builder.getUnknownLoc(), graphSymbol, loopa);
  } else {
    loopOp = mlir::starplat::ForAllNeighboursOp::create(
        builder, builder.getUnknownLoc(), graphSymbol, nodeSymbol, loopa);
  }
  ...
  if (hasFilter) {
    ...
    if (lhsExprKind == ExpressionKind::KIND_MEMBERACCESS) {
      mlir::Value nodeVal = lhsIsLoopVar ? loopBlock.getArgument(0)
                                         : globalLookupOp(lhsFilterName);
      if (!nodeVal) {
        llvm::outs() << "Error: Undefined node '" << lhsFilterName
                     << "' in filter.\n";
        return;
      }
      mlir::Type propElemType = builder.getI64Type();
      if (auto propNodeType = mlir::dyn_cast<mlir::starplat::PropNodeType>(
              lhsPropVal.getType()))
        propElemType = propNodeType.getParameter();
      lhsFilterSymbol =
          mlir::starplat::GetNodePropertyOp::create(
              builder, builder.getUnknownLoc(), propElemType, nodeVal,
              lhsPropVal, builder.getStringAttr(lhsPropName))
              ->getResult(0);
      ...
      if (!lhsFilterSymbol) {
        llvm::outs() << "Error: Undefined identifier '" << lhsFilterName
                     << "' in filter.\n";
        return;
      }
    }
    ...
    if (rhsIsLoopVar) {
      ...
      if (!rhsFilterSymbol) {
        llvm::outs() << "Error: Undefined identifier '" << rhsFilterName
                     << "' in filter.\n";
        return;
      }
    }
    ...
    auto cmpOp = mlir::starplat::CmpOp::create(
        builder, builder.getUnknownLoc(),
        mlir::IntegerType::get(builder.getContext(), 1), lhsFilterSymbol,
        rhsFilterSymbol, builder.getStringAttr(filterOp));

    auto ifOp = mlir::starplat::StarPlatIfOp::create(
        builder, builder.getUnknownLoc(), cmpOp->getResult(0));
...
void StarPlatCodeGen::visitForStmt(const ForStatement *forStmt,
                                   mlir::SymbolTable *symbolTable) {
  ...
  const Statementlist *stmtlist =
      static_cast<const Statementlist *>(forStmt->getstmtlist());
  ...
  mlir::Value graphSymbol =
      globalLookupOp(memberaccess->getIdentifier()->getname());
  if (!graphSymbol) {
  ...
  if (strcmp(methodcall->getIdentifier()->getname(), "nodes_to") == 0) {
    const Expression *paramExpr =
        static_cast<const Expression *>(methodcall->getParamLists());
    const Identifier *paramId =
        static_cast<const Identifier *>(paramExpr->getExpression());
    nodeSymbol = globalLookupOp(paramId->getname());
    if (!nodeSymbol) {
    ...
    llvm::outs() << "Error: Unknown method '"
                 << methodcall->getIdentifier()->getname()
                 << "' in for stmt.\n";
    return;
  }
  ...
  // mlir::ArrayAttr loopAttr                   =
  // builder.getArrayAttr({builder.getStringAttr("nodes_to")}); mlir::BoolAttr
  // filterAttr                  = builder.getBoolAttr(false);
  auto forOp = mlir::starplat::ForNodesToOp::create(
      builder, builder.getUnknownLoc(), graphSymbol, nodeSymbol, loopa);
...
void StarPlatCodeGen::visitDoWhileStmt(const DoWhileStatement *doWhileStmt,
                                       mlir::SymbolTable *symbolTable) {
  const Expression *expr =
      static_cast<const Expression *>(doWhileStmt->getexpr());
  const Statementlist *stmtlist =
      static_cast<const Statementlist *>(doWhileStmt->getstmtlist());

  auto doWhileOp = mlir::starplat::DoWhileOp::create(
      builder, builder.getUnknownLoc(), builder.getStringAttr("DoWhile"));
...
void StarPlatCodeGen::visitMemberaccessStmt(
    const MemberacceessStmt *MemberacceessStmt,
    mlir::SymbolTable *symbolTable) {
  const Memberaccess *memberaccessnode =
      static_cast<const Memberaccess *>(MemberacceessStmt->getMemberAccess());
  const Methodcall *methodcallnode =
      static_cast<const Methodcall *>(memberaccessnode->getMethodCall());
  ...
  const Paramlist *paramlist =
      static_cast<const Paramlist *>(methodcallnode->getParamLists());
  ...
  if (methodcallnode && methodcallnode->getIsBuiltin()) {
    if (std::strcmp(methodcallnode->getIdentifier()->getname(),
                    "attachNodeProperty") == 0) {
      ...
      for (Param *param : paramListVector) {
        if (!param->getParamAssignment())
          continue;
        const auto *paramAssignment = static_cast<const ParameterAssignment *>(
            param->getParamAssignment());
        if (!paramAssignment)
          continue;
        const auto *identifier =
            static_cast<const Identifier *>(paramAssignment->getidentifier());
        auto idSymbol = globalLookupOp(identifier->getname());
        if (!idSymbol) {
          llvm::outs() << "Error: Identifier '" << identifier->getname()
                       << "' not declared.\n";
          return;
        }
        ...
        if (paramAssignment->isKeyword()) {
          const auto *keyword =
              static_cast<const Keyword *>(paramAssignment->getkeyword());
          keyword->Accept(this, symbolTable);
          kwSymbol = globalLookupOp(keyword->getKeyword());
        } else {
          const auto *expr =
              static_cast<const Expression *>(paramAssignment->getexpr());
      ...
      mlir::starplat::AttachNodePropertyOp::create(
          builder, builder.getUnknownLoc(), operandsForAttachNodeProperty);
    } else {
      llvm::errs() << methodcallnode->getIdentifier()->getname()
                   << " is not implemented yet\n";
    }
  }
}

void StarPlatCodeGen::visitIfStmt(const IfStatement *ifStmt,
                                  mlir::SymbolTable *symbolTable) {
  const Expression *condExpr =
      static_cast<const Expression *>(ifStmt->getexpr());
  ...
  if (condExpr->getKind() == ExpressionKind::KIND_MEMBERACCESS) {
    const Memberaccess *memberaccess =
        static_cast<const Memberaccess *>(condExpr->getExpression());
    const Methodcall *methodcall =
        static_cast<const Methodcall *>(memberaccess->getMethodCall());
    ...
    if (methodcall && methodcall->getIsBuiltin() &&
      ...
      const Methodcall *methodcall =
          static_cast<const Methodcall *>(memberaccess->getMethodCall());
      const Paramlist *paramlist =
          static_cast<const Paramlist *>(methodcall->getParamLists());
      auto params = paramlist->getParamList();

      const Identifier *id1 = static_cast<const Identifier *>(
          params[0]->getExpr()->getExpression());
      const Identifier *id2 = static_cast<const Identifier *>(
          params[1]->getExpr()->getExpression());
      ...
      cond = mlir::starplat::IsEdgeOp::create(
          builder, builder.getUnknownLoc(),
          mlir::IntegerType::get(builder.getContext(), 1), graphSymbol,
  ...
  auto ifOp = mlir::starplat::StarPlatIfOp::create(
      builder, builder.getUnknownLoc(), cond);
...
void StarPlatCodeGen::visitIfElseStmt(const IfElseStatement *ifElseStmt,
                                      mlir::SymbolTable *symbolTable) {
  const Expression *condExpr =
      static_cast<const Expression *>(ifElseStmt->getexpr());
  ...
  // Create IfElseOp
  auto ifElseOp = mlir::starplat::StarPlatIfElseOp::create(
      builder, builder.getUnknownLoc(), cond, builder.getStringAttr("ifelse"));
...
void StarPlatCodeGen::visitBoolExpr(const BoolExpr *boolExpr,
                                    mlir::SymbolTable *symbolTable) {}

void StarPlatCodeGen::visitIncandassignstmt(const Incandassignstmt *stmt,
                                            mlir::SymbolTable *symbolTable) {
  auto lhsSymbol = globalLookupOp(stmt->getIdentifier()->getname());
  if (!lhsSymbol) {
    llvm::outs() << "Error: Identifier '" << stmt->getIdentifier()->getname()
                 << "' not declared.\n";
    return;
  }
...
void StarPlatCodeGen::visitIncstmt(const Incstmt *incstmt,
                                   mlir::SymbolTable *symbolTable) {
  auto lhsSymbol = globalLookupOp(incstmt->getIdentifier()->getname());
  if (!lhsSymbol) {
    llvm::outs() << "Error: Identifier '" << incstmt->getIdentifier()->getname()
                 << "' not declared.\n";
    return;
  }
...
void StarPlatCodeGen::visitAssignmentStmt(const AssignmentStmt *assignemntStmt,
                                          mlir::SymbolTable *symbolTable) {
  Assignment *assignment =
      static_cast<Assignment *>(assignemntStmt->getAssignment());
...
void StarPlatCodeGen::visitIdentifier(const Identifier *identifier,
                                      mlir::SymbolTable *symbolTable) {}

void StarPlatCodeGen::visitReturnStmt(const ReturnStmt *returnStmt,
                                      mlir::SymbolTable *symbolTable) {
  const Expression *expr =
      static_cast<const Expression *>(returnStmt->getexpr());
  ...
  if (expr->getKind() == KIND_NUMBER) {
    ...
    mlir::starplat::ReturnOp::create(builder, builder.getUnknownLoc(),
                                     mlir::ValueRange{retSymbol});
  }

  else if (expr->getKind() == KIND_IDENTIFIER) {
    const Identifier *identifier =
        static_cast<const Identifier *>(expr->getExpression());

    auto retSymbol = globalLookupOp(identifier->getname());
    if (!retSymbol) {
      llvm::outs() << "Error: Return value '" << identifier->getname()
                   << "' not found.\n";
      mlir::starplat::ReturnOp::create(builder, builder.getUnknownLoc(),
                                       mlir::ValueRange{});
      return;
...
void StarPlatCodeGen::visitParameterAssignment(
    const ParameterAssignment *paramAssignment,
    mlir::SymbolTable *symbolTable) {
  Identifier *identifier =
      static_cast<Identifier *>(paramAssignment->getidentifier());
  if (paramAssignment->isKeyword()) {
...
void StarPlatCodeGen::visitParam(const Param *param,
                                 mlir::SymbolTable *symbolTable) {
  const ParameterAssignment *paramAssignment =
      static_cast<const ParameterAssignment *>(param->getParamAssignment());
  if (paramAssignment != nullptr) {
    paramAssignment->Accept(this, symbolTable);
  }
}

void StarPlatCodeGen::visitTupleAssignment(
    const TupleAssignment *tupleAssignment, mlir::SymbolTable *symbolTable) {

  const Expression *lhsexpr1 =
      static_cast<const Expression *>(tupleAssignment->getlhsexpr1());
  const Expression *lhsexpr2 =
      static_cast<const Expression *>(tupleAssignment->getlhsexpr2());
  const Expression *rhsexpr1 =
      static_cast<const Expression *>(tupleAssignment->getrhsexpr1());
  const Expression *rhsexpr2 =
      static_cast<const Expression *>(tupleAssignment->getrhsexpr2());

  mlir::Value lhsNode1, lhsProp1, lhsNode2, lhsProp2;

  // ---- LHS1 ----
  if (lhsexpr1->getKind() == ExpressionKind::KIND_MEMBERACCESS) {
    const Memberaccess *lhs1 =
        static_cast<const Memberaccess *>(lhsexpr1->getExpression());
  ...
  if (lhsexpr2->getKind() == ExpressionKind::KIND_MEMBERACCESS) {
    const Memberaccess *lhs2 =
        static_cast<const Memberaccess *>(lhsexpr2->getExpression());
  ...
  const Methodcall *rhsMethodCall =
      static_cast<const Methodcall *>(rhsexpr1->getExpression());
  if (!rhsMethodCall->getIsBuiltin() ||
      strcmp(rhsMethodCall->getIdentifier()->getname(), "Min") != 0) {
    llvm::outs() << rhsMethodCall->getIdentifier()->getname()
                 << " Not implemented yet.\n";
    return;
  }
  const Paramlist *rhsParamList =
      static_cast<const Paramlist *>(rhsMethodCall->getParamLists());
  const Expression *minArg1 = static_cast<const Expression *>(
      rhsParamList->getParamList()[0]->getExpr());
  const Expression *minArg2 = static_cast<const Expression *>(
      rhsParamList->getParamList()[1]->getExpr());
  ...
  // ---- Create MinOp ----
  mlir::starplat::MinOp::create(builder, builder.getUnknownLoc(), lhsNode1,
                                lhsProp1, lhsNode2, lhsProp2, rhsValue1,
                                rhsValue2, updateValue);
}
...
void StarPlatCodeGen::visitAnd(const And *_and,
                               mlir::SymbolTable *symbolTable) {}

void StarPlatCodeGen::visitOr(const Or *_or, mlir::SymbolTable *symbolTable) {}

void StarPlatCodeGen::visitFunction(const Function *function,
                                    mlir::SymbolTable *symbolTable) {
  ...
  for (auto arg : args) {
    if (arg->getType() != nullptr) {
      if (std::string(arg->getType()->getType()) == "Graph") {
        ...
        argTypes.push_back(
            mlir::starplat::SPFloatType::get(builder.getContext()));
      } else if (std::string(arg->getType()->getType()) == "int") {
        argTypes.push_back(
            mlir::starplat::SPIntType::get(builder.getContext()));
      }
    } else if (arg->getTemplateType() != nullptr) {
      llvm::StringRef graphId =
          arg->getTemplateType()->getGraphName()->getname();
      ...
      else if (std::string(arg->getTemplateType()->getType()->getType()) ==
               "bool")
        elemType = builder.getI1Type();

      if (std::string(
              arg->getTemplateType()->getGraphPropNode()->getPropertyType()) ==
          "propNode") {
        argTypes.push_back(
            builder.getType<mlir::starplat::PropNodeType>(elemType, graphId));
      } else if (std::string(arg->getTemplateType()
                                 ->getGraphPropNode()
                                 ->getPropertyType()) == "propEdge") {
        argTypes.push_back(
            builder.getType<mlir::starplat::PropEdgeType>(elemType, graphId));
      }
    }
  ...
  auto func = mlir::starplat::FuncOp::create(builder, builder.getUnknownLoc(),
                                             function->getfuncNameIdentifier(),
                                             funcType, argNamesAttr);
  // func.setNested();
  ...
  Statementlist *stmtlist =
      static_cast<Statementlist *>(function->getstmtlist());
...
void StarPlatCodeGen::visitParamlist(const Paramlist *paramlist,
                                     mlir::SymbolTable *symbolTable) {
...
void StarPlatCodeGen::visitFixedpointUntil(
    const FixedpointUntil *fixedpointuntil, mlir::SymbolTable *symbolTable) {
  // Create new region.
  ...
  Identifier *identifier =
      static_cast<Identifier *>(fixedpointuntil->getidentifier());
  Expression *expr = static_cast<Expression *>(fixedpointuntil->getexpr());
  Statementlist *stmtlist =
      static_cast<Statementlist *>(fixedpointuntil->getstmtlist());
  ...
  const Expression *innerBoolExpr =
      static_cast<const Expression *>(boolExpr->getExpr1());
  mlir::Value rhs;
  if (innerBoolExpr->getKind() == ExpressionKind::KIND_IDENTIFIER) {
    rhs = globalLookupOp(
        static_cast<const Identifier *>(innerBoolExpr->getExpression())
            ->getname());
    if (!rhs) {
      llvm::errs() << "Error: "
                   << static_cast<const Identifier *>(
                          innerBoolExpr->getExpression())
                          ->getname()
                   << " not declared.\n";
      return;
    }
  ...
  auto fixedPointUntil = mlir::starplat::FixedPointUntilOp::create(
      builder, builder.getUnknownLoc(), condArgs, condAttrArray, fixPntAttr);
...
void StarPlatCodeGen::visitInitialiseAssignmentStmt(
    const InitialiseAssignmentStmt *initialiseAssignmentStmt,
    mlir::SymbolTable *symbolTable) {
  const TypeExpr *type =
      static_cast<const TypeExpr *>(initialiseAssignmentStmt->gettype());
  const Identifier *identifier = static_cast<const Identifier *>(
      initialiseAssignmentStmt->getidentifier());
  const Expression *expr =
      static_cast<const Expression *>(initialiseAssignmentStmt->getexpr());
  ...
  auto idDecl = mlir::starplat::DeclareOp2::create(
      builder, builder.getUnknownLoc(), typeAttr,
      builder.getStringAttr(identifier->getname()),
      builder.getStringAttr("public"));
  ...
  if (expr->getKind() == ExpressionKind::KIND_KEYWORD) {
    const Keyword *keyword =
        static_cast<const Keyword *>(expr->getExpression());
    ...
    const Memberaccess *memberAccessIn =
        static_cast<const Memberaccess *>(expr->getExpression());
    const Identifier *identifierIn =
        static_cast<const Identifier *>(memberAccessIn->getIdentifier());
    const Methodcall *methodcallIn =
        static_cast<const Methodcall *>(memberAccessIn->getMethodCall());
    ...
    if (methodcallIn->getIsBuiltin()) {
      if (strcmp(methodcallIn->getIdentifier()->getname(), "get_edge") == 0) {
        // Visit ParamList
        const Paramlist *paramlist =
            static_cast<const Paramlist *>(methodcallIn->getParamLists());
        paramlist->Accept(this, symbolTable);

        std::vector<Param *> paramlistvector = paramlist->getParamList();
        const Identifier *node1 = static_cast<const Identifier *>(
            paramlistvector[0]->getExpr()->getExpression());
        const Identifier *node2 = static_cast<const Identifier *>(
            paramlistvector[1]->getExpr()->getExpression());

        auto node1Op = globalLookupOp(node1->getname());
        auto node2Op = globalLookupOp(node2->getname());

        // Create a get_edge Op.
        auto getedgeOp = mlir::starplat::GetEdgeOp::create(
            builder, builder.getUnknownLoc(), typeAttr, accessIdentifier,
            node1Op, node2Op);
        ...
        mlir::starplat::AssignmentOp::create(builder, builder.getUnknownLoc(),
                                             lhs, rhs);
      } else if (strcmp(methodcallIn->getIdentifier()->getname(),
                        "num_nodes") == 0) {
...
void StarPlatCodeGen::visitMemberAccessAssignment(
    const MemberAccessAssignment *memberAccessAssignment,
    mlir::SymbolTable *symbolTable) {

  const Memberaccess *memberAccess = static_cast<const Memberaccess *>(
      memberAccessAssignment->getMemberAccess());
  memberAccess->Accept(this, symbolTable);

  const Expression *expr =
      static_cast<const Expression *>(memberAccessAssignment->getExpr());
  expr->Accept(this, symbolTable);
  ...
  if (mlir::isa<mlir::starplat::NodeType>(type)) {
    ...
    if (expr->getKind() == ExpressionKind::KIND_NUMBER) {
      ...
      if (id2.getDefiningOp() != nullptr)
        // auto setNodeProp =
        mlir::starplat::SetNodePropertyOp::create(
            builder, builder.getUnknownLoc(),
            id2.getDefiningOp()->getOperand(0), id1, id2, numberVal);
      else
        // auto setNodeProp =
        mlir::starplat::SetNodePropertyOp::create(
            builder, builder.getUnknownLoc(), id2, id1, id2, numberVal);
    }
      ...
      // auto setNodeProp =
      mlir::starplat::SetNodePropertyOp::create(
          builder, builder.getUnknownLoc(), id2.getDefiningOp()->getOperand(0),
          id1, id2, keywordVal);
    }
      ...
      if (!rhs) {
        llvm::outs()
            << "Error: Failed to resolve RHS in member access assignment.\n";
        return;
      }
      mlir::starplat::SetNodePropertyOp::create(
          builder, builder.getUnknownLoc(), id2.getDefiningOp()->getOperand(0),
...
void StarPlatCodeGen::visitKeyword(const Keyword *keyword,
                                   mlir::SymbolTable *symbolTable) {
  ...
  if (strcmp(keyword->getKeyword(), "False") == 0)
    keywordSSA = mlir::starplat::ConstOp::create(
        builder, builder.getUnknownLoc(), builder.getI1Type(),
        builder.getStringAttr(keyword->getKeyword()),
        builder.getStringAttr(keyword->getKeyword()),
        builder.getStringAttr("public"));
...
void StarPlatCodeGen::visitGraphProperties(
    const GraphProperties *graphproperties, mlir::SymbolTable *symbolTable) {}

void StarPlatCodeGen::visitMethodcall(const Methodcall *methodcall,
                                      mlir::SymbolTable *symbolTable) {
  const Paramlist *paramlist =
      static_cast<const Paramlist *>(methodcall->getParamLists());
  paramlist->Accept(this, symbolTable);
...
void StarPlatCodeGen::visitMemberaccess(const Memberaccess *memberaccess,
                                        mlir::SymbolTable *symbolTable) {

  if (!globalLookupOp(
          builder.getStringAttr(memberaccess->getIdentifier()->getname()))) {
    if (globalLookupOp(memberaccess->getIdentifier()->getname()) == nullptr) {
      llvm::outs() << "Error1: " << memberaccess->getIdentifier()->getname()
                   << " not defined!\n";
      exit(0);
    }
  }
  ...
  if (memberaccess->getIdentifier2()) {
    if (!globalLookupOp(
            builder.getStringAttr(memberaccess->getIdentifier2()->getname()))) {
      if (globalLookupOp(memberaccess->getIdentifier()->getname()) == nullptr) {
        llvm::outs() << "Error2: " << memberaccess->getIdentifier2()->getname()
                     << " not defined!\n";
        exit(0);
      }
    }
...
void StarPlatCodeGen::visitArglist(const Arglist *arglist,
                                   mlir::SymbolTable *symbolTable) {}

void StarPlatCodeGen::visitArg(const Arg *arg, mlir::SymbolTable *symbolTable) {
}

void StarPlatCodeGen::visitStatement(const Statement *statement,
                                     mlir::SymbolTable *symbolTable) {}

void StarPlatCodeGen::visitStatementlist(const Statementlist *stmtlist,
                                         mlir::SymbolTable *symbolTable) {
...
void StarPlatCodeGen::visitType(const TypeExpr *type,
                                mlir::SymbolTable *symbolTable) {}

void StarPlatCodeGen::visitNumber(const Number *number,
                                  mlir::SymbolTable *symbolTable) {
  if (number->isFloat()) {
    std::string key = std::to_string(number->getnumberfloat());
    if (globalLookupOp(key))
      return;
    auto constant = mlir::starplat::ConstOp::create(
        builder, builder.getUnknownLoc(), builder.getF64Type(),
    ...
    auto constant = mlir::starplat::ConstOp::create(
        builder, builder.getUnknownLoc(), builder.getI64Type(),
        builder.getStringAttr(key), builder.getStringAttr(key),
        builder.getStringAttr("public"));
    symbolTable->insert(constant);
  }
}

void StarPlatCodeGen::visitExpression(const Expression *expr,
                                      mlir::SymbolTable *symbolTable) {
...
mlir::SymbolTable *StarPlatCodeGen::getSymbolTable() {
...
mlir::Value StarPlatCodeGen::globalLookupOp(llvm::StringRef name) {
  // llvm::outs() << "Looking up: " << name << " in " << symbolTables.size() <<
  // " symbol tables\n";
...
mlir::Value StarPlatCodeGen::resolveExpr(const Expression *expr,
                                         mlir::SymbolTable *symbolTable) {
  if (!expr) {
  ...
  case ExpressionKind::KIND_NUMBER: {
    ...
    if (num->isFloat()) {
      ...
      auto constOp = mlir::starplat::ConstOp::create(
          builder, builder.getUnknownLoc(), floatType, constAttr,
  ...
  case ExpressionKind::KIND_MEMBERACCESS: {
    ...
    const Identifier *id1 =
        static_cast<const Identifier *>(mem->getIdentifier());
    ...
    if (mem->getMethodCall()) {
      const Methodcall *method =
          static_cast<const Methodcall *>(mem->getMethodCall());
      ...
      if (method->getIsBuiltin()) {
        if (strcmp(method->getIdentifier()->getname(), "count_outNbrs") == 0) {
          ...
          return mlir::starplat::CountOutNbrsOp::create(
                     ...
                     objVal, nodeVal)
              ->getResult(0);
        } else if (strcmp(method->getIdentifier()->getname(), "is_an_edge") ==
                   0) {
          const Paramlist *paramlist =
              static_cast<const Paramlist *>(method->getParamLists());
          auto params = paramlist->getParamList();
          const Identifier *id1 = static_cast<const Identifier *>(
              params[0]->getExpr()->getExpression());
          const Identifier *id2 = static_cast<const Identifier *>(
          ...
          return mlir::starplat::IsEdgeOp::create(
                     builder, builder.getUnknownLoc(),
                     mlir::IntegerType::get(builder.getContext(), 1), objVal,
                     node1, node2)
              ->getResult(0);
        }
      }
      llvm::outs() << "Error: Unsupported method call '"
                   << method->getIdentifier()->getname()
                   << "' in resolveExpr\n";
      return nullptr;
    }
    ...
    const Identifier *id2 =
        static_cast<const Identifier *>(mem->getIdentifier2());
    if (!id2) {
      llvm::outs() << "Error: Malformed member access\n";
      return nullptr;
    }
    ...
    if (!nodeVal || !propVal) {
      llvm::outs() << "Error: Undefined member access '" << id1->getname()
                   << "." << id2->getname() << "'\n";
      return nullptr;
    }
    ...
    if (mlir::isa<mlir::starplat::NodeType>(nodeVal.getType())) {
      mlir::Type propElemType = builder.getI64Type();
      if (auto propNodeType =
              mlir::dyn_cast<mlir::starplat::PropNodeType>(propVal.getType()))
        propElemType = propNodeType.getParameter();
      return mlir::starplat::GetNodePropertyOp::create(
                 builder, builder.getUnknownLoc(), propElemType, nodeVal,
                 propVal, builder.getStringAttr(id2->getname()))
          ->getResult(0);
    }

    if (mlir::isa<mlir::starplat::EdgeType>(nodeVal.getType())) {
      mlir::Type propElemType = builder.getI64Type();
      if (auto propEdgeType =
              mlir::dyn_cast<mlir::starplat::PropEdgeType>(propVal.getType()))
  ...
  case ExpressionKind::KIND_ADDOP: {
    ...
    auto lhs = resolveExpr(static_cast<const Expression *>(op->getOperand1()),
                           symbolTable);
    auto rhs = resolveExpr(static_cast<const Expression *>(op->getOperand2()),
                           symbolTable);
    if (!lhs || !rhs)
      return nullptr;
    return mlir::starplat::AddOp::create(builder, builder.getUnknownLoc(),
                                         inferType(lhs, rhs), lhs, rhs)
        ->getResult(0);
  }
  ...
  case ExpressionKind::KIND_BOOLEXPR: {
    const BoolExpr *boolExpr =
        static_cast<const BoolExpr *>(expr->getExpression());
    const Expression *lhsExpr =
        static_cast<const Expression *>(boolExpr->getExpr1());
    const Expression *rhsExpr =
        static_cast<const Expression *>(boolExpr->getExpr2());
    ...
    if (!lhs || !rhs)
      return nullptr;
    return mlir::starplat::CmpOp::create(
               builder, builder.getUnknownLoc(),
               mlir::IntegerType::get(builder.getContext(), 1), lhs, rhs,
