...
class graph
{
  ...
  public:
    ...
    edge* getEdge(int s, int d) {
        ...
        edge* e = new edge();
        for (int i = indexofNodes[s]; i < indexofNodes[s + 1]; i++) {
            int nbr = edgeList[i];
            if (nbr != d)
                continue;
            if (nbr != INT_MAX / 2) {
        ...
        return e; // TODO: Maybe return a default value?
    }
    ...
    int getOutDegree(int v) { return outDeg[v]; }

    int getInDegree(int v) { return inDeg[v]; }
...
#pragma omp parallel for num_threads(4)
    ...
    //         }
    //
    //         updateCSRDel_omp(num_nodes, edgesTotal, indexofNodes, edgeList, rev_indexofNodes, srcList, edgeLen, diff_edgeLen, diff_indexofNodes,
    //                          diff_edgeList, diff_rev_indexofNodes, diff_rev_edgeList, rev_edgeLen, diff_rev_edgeLen, perNodeUpdateInfo,
    //                          perNodeUpdateRevInfo, perNodeCSRSpace, perNodeRevCSRSpace, slicedUpdates);
    //
    ...
    //         }
    //
    //         updateCSRAdd_omp(num_nodes, edgesTotal, indexofNodes, edgeList, rev_indexofNodes, srcList, edgeLen, &diff_edgeLen, &diff_indexofNodes,
    //                          &diff_edgeList, &diff_rev_indexofNodes, &diff_rev_edgeList, rev_edgeLen, &diff_rev_edgeLen, perNodeUpdateInfo,
    //                          perNodeUpdateRevInfo, perNodeCSRSpace, perNodeRevCSRSpace, slicedUpdates);
    //
    //         perNodeUpdateInfo.clear();
